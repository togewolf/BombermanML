...
def state_to_features(game_state: dict, logger) -> np.array:
    ...
    if danger_zone[ax, ay] > 0:
        in_danger = True
    # nearest_safe = get_safe_tile_distances(ax, ay, danger_zone, others, bombs, dist, grad, in_danger)
...
def danger_map(game_state):
    ...
    field = game_state['field']
    explosion_map = np.copy(game_state['danger_map'])
    ...
    if (ax, ay) in bombs:  # is standing on a bomb, bombs cannot be empty if this is the case
        ...
        for i, (dx, dy) in enumerate(directions):
            if field[dx, dy] != 0:  # Danger or obstacle ahead
                disallowed[i] = 1
        # todo: prevent going into direction that the enemy can block

    # disallow bomb placement if it would lead to certain death
    if bomb_available:
        # todo: do not drop a bomb if all escape routes could be blocked by an enemy
...
def do_not_enter_dead_end(ax, ay, dead_end_map, dead_end_list, others, dist, grad):
    ...
    if 2 * distance_to_exit + 1 >= nearest_enemy_distance:
        # Step 4: Use the get_direction_to_object to find the direction toward the exit
...
if goal is crate and it has reached it, encourage it to drop a bomb and not just hang around there for a few steps
only suggest crates if there could be a coin in that crate

Somehow reward blocking enemies and causing them to kill themselves (mostly solved with dead end and forcing
the enemy to drop a bomb then if possible)
